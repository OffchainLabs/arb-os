...
fn mavm_codegen_expr<'a>(
    ...
    match expr {
        ...
        TypeCheckedExpr::CopyBuffer8(tce1, tce2, tce3, tce4, tce5, loc) => {
            let (lg, c, locals5) = mavm_codegen_expr(
                tce5,
                code,
                num_locals,
                locals,
            ...
            let (lg, c, locals4) = mavm_codegen_expr(
                tce4,
                c,
                num_locals,
                locals,
            ...
            label_gen = lg;
            code = c;
            code.push(Instruction::from_opcode(Opcode::AVMOpcode(AVMOpcode::CopyBuffer8), *loc));
            Ok((
                label_gen,
                code,
                max(num_locals, max(locals1, max(locals2, max(locals3, max(locals4, locals5))))),
            ))
        }
