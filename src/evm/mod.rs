...
pub fn evm_test_arbsys_direct(log_to: Option<&Path>, debug: bool) -> Result<(), ethabi::Error> {
    ...
    let (return_receipts, _) = add_contract.call_function(
        my_addr.clone(),
        "expectedReturnDirect",
        &[],
        &mut machine,
        Uint256::zero(),
        debug,
    ...
    assert_eq!(
        return_receipts[0].get_return_data(),
        Uint256::zero().to_bytes_be()
    );

    let (return_receipts_uint_noret, _) = add_contract.call_function(
        my_addr.clone(),
        "expectedReturnUintNoReturn",
        &[],
        &mut machine,
        Uint256::zero(),
        debug,
