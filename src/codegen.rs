...
pub fn mavm_codegen<'a>(
	...
	for func in funcs {
		if ! func.imported {
			let (lg, c) = mavm_codegen_func(func, code, label_gen, string_table, &import_func_map, &global_var_map)?;
			label_gen = lg;
			code = c;
		}
	}
...
fn add_args_to_locals_table<'a>(
	...
	if args.is_empty() {
		mavm_codegen_statements(statements, code, num_locals, &locals, label_gen, string_table, import_func_map, global_var_map)
	} else {
	...
	match &statements[0] {
		...
		TypeCheckedStatement::Let(pat, expr, loc) => match pat {
			TypeCheckedMatchPattern::Simple(name, _) => {
				...
				label_gen = lg;
				code = c;
				code.push(Instruction::from_opcode_imm(Opcode::SetLocal, Value::Int(Uint256::from_usize(slot_num)), *loc)); 
				mavm_codegen_statements(
					rest_of_statements.to_vec(), 
					code, 
					num_locals, 
		...
		TypeCheckedStatement::While(cond, body, loc) => {
			...
			label_gen = lg;
			code.push(Instruction::from_opcode_imm(Opcode::Noop, Value::Label(top_label), *loc));
			code.push(Instruction::from_opcode_imm(Opcode::SetLocal, slot_num, *loc));
			code.push(Instruction::from_opcode_imm(Opcode::Jump, Value::Label(cond_label), *loc));
			code.push(Instruction::from_opcode(Opcode::Label(top_label), *loc));
			let (lg, nl, _) = mavm_codegen_statements(
				body.to_vec(), 
				code, 
			...
			let (lg, c) = mavm_codegen_expr(cond, code, &locals, label_gen, string_table, import_func_map, global_var_map)?;
			label_gen = lg;
			code = c;
			code.push(Instruction::from_opcode_imm(Opcode::Cjump, Value::Label(top_label), *loc));
...
fn mavm_codegen_if_arm<'a>(
	...
	match arm {
		TypeCheckedIfArm::Cond(cond, body, orest, loc) => {
			...
			let (lg, c) = mavm_codegen_expr(cond, code, &locals, lg, string_table, import_func_map, global_var_map)?;
			label_gen = lg;
			code = c;
			code.push(Instruction::from_opcode(Opcode::Not, *loc));
			code.push(Instruction::from_opcode_imm(Opcode::Cjump, Value::Label(after_label), *loc));
	...
	match expr {
		TypeCheckedExpr::UnaryOp(op, tce, _, loc) => {
			...
			let (maybe_opcode, maybe_imm) = match op {
				...
				UnaryOp::ToAddress => {
					let mask = Uint256::from_usize(2).exp(&Uint256::from_usize(20)).sub(&Uint256::one());
		...
		TypeCheckedExpr::Binary(op, tce1, tce2, _, loc) => {
			let (lg, c) = mavm_codegen_expr(tce2, code, locals, label_gen, string_table, import_func_map, global_var_map)?;
			let (lg, c) = mavm_codegen_expr(tce1, c, locals, lg, string_table, import_func_map, global_var_map)?;
			label_gen = lg;
			code = c;
			let opcode = match op {     
			...
			match op {
				...
				BinaryOp::GreaterEq |
				BinaryOp::SLessEq |
				BinaryOp::SGreaterEq => { code.push(Instruction::from_opcode(Opcode::Not, *loc)); }
				_ => {}
			}
			Ok((label_gen, code))
		}
		TypeCheckedExpr::ShortcutOr(tce1, tce2, loc) => {
			let (lg, c) = mavm_codegen_expr(tce1, code, locals, label_gen, string_table, import_func_map, global_var_map)?;
			let (lab, lg) = lg.next();
			c.push(Instruction::from_opcode(Opcode::Dup0, *loc));
			c.push(Instruction::from_opcode_imm(Opcode::Cjump, Value::Label(lab), *loc));
			c.push(Instruction::from_opcode(Opcode::Pop, *loc));
			let (lg, c) = mavm_codegen_expr(tce2, c, locals, lg, string_table, import_func_map, global_var_map)?;
		...
		TypeCheckedExpr::LocalVariableRef(name, _, loc) => {
			match locals.get(*name) {
				Some(n) => {
					code.push(Instruction::from_opcode_imm(Opcode::GetLocal, Value::Int(Uint256::from_usize(n)), *loc)); 
					Ok((label_gen, code))
				}
				None => {
					println!("local: {:?}", *name);
		...
		TypeCheckedExpr::Const(val, _, loc) => {
			code.push(Instruction::from_opcode_imm(Opcode::Noop, val.clone(), *loc));
			Ok((label_gen, code))			
		}
		...
		TypeCheckedExpr::StructInitializer(fields, _, loc) => {
			...
			for i in 0..fields_len {
				...
				let (lg, c) = mavm_codegen_expr(&field.value, code, locals, label_gen, string_table, import_func_map, global_var_map)?;
				label_gen = lg;
				code = c;
			}
			let empty_vec = vec![Value::none(); fields_len];
			code.push(Instruction::from_opcode_imm(Opcode::Noop, Value::Tuple(empty_vec), *loc));
			for i in 0..fields_len {
				code.push(Instruction::from_opcode_imm(Opcode::TupleSet(fields_len), Value::Int(Uint256::from_usize(i)), *loc));
			}
			Ok((label_gen, code))
		}
		TypeCheckedExpr::Tuple(fields, _, loc) => {
			...
			for i in 0..fields_len {
				let field = &fields[fields_len-1-i];
				let (lg, c) = mavm_codegen_expr(&field, code, locals, label_gen, string_table, import_func_map, global_var_map)?;
				label_gen = lg;
				code = c;
			}
			let empty_vec = vec![Value::none(); fields_len];
		...
		TypeCheckedExpr::FixedArrayRef(expr1, expr2, size, _, loc) => {
			let (lg, c) = mavm_codegen_expr(expr1, code, locals, label_gen, string_table, import_func_map, global_var_map)?;
			let (lg, c) = mavm_codegen_expr(expr2, c, locals, lg, string_table, import_func_map, global_var_map)?;
			label_gen = lg;
			code = c;
		...
		TypeCheckedExpr::NewFixedArray(sz, bo_expr, _, loc) => {
			match bo_expr {
				Some(expr) => {
					...
					code.push(Instruction::from_opcode_imm(Opcode::Noop,Value::Tuple(empty_tuple), *loc));
					for i in 0..8 {
						code.push(Instruction::from_opcode_imm(Opcode::Tset, Value::Int(Uint256::from_usize(i)), *loc));
					}
				}
				None => {
					let empty_tuple = vec![Value::Tuple(Vec::new()); 8];
		...
		TypeCheckedExpr::StructMod(struc, index, val, t, loc) => {
			...
			if let Type::Struct(v) = t {
				...
				code.push(Instruction::from_opcode_imm(Opcode::TupleSet(struct_len), Value::Int(Uint256::from_usize(*index)), *loc));
			} else {
...
fn codegen_fixed_array_mod<'a>(
	...
	codegen_fixed_array_mod_2(val_expr, size, code, locals, label_gen, string_table, import_func_map, global_var_map, location)
}
